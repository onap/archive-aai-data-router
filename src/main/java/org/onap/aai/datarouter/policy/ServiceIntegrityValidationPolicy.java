...
/**
 * This class handles the logic which transformers the POA-AUDIT-RESULT event message to the
 * ElasticSearch validation/violation message in order to prepare two Json structures, one for
 * validation index and one for violation index and then submit POST request to Search-Data-Service
 * to insert the document into ES.
 *
 */
public class ServiceIntegrityValidationPolicy {
    ...
    private static Logger logger =
            LoggerFactory.getInstance().getLogger(ServiceIntegrityValidationPolicy.class.getSimpleName());
    ...
    public ServiceIntegrityValidationPolicy(String searchCertPath, String searchCertTruststore,
            String searchCertPassword, String searchBaseURL, String endpoint, String validationIndexName,
            String violationIndexName) {
        searchAgent = new SearchServiceAgent(searchCertPath, searchCertTruststore, searchCertPassword,
                concatSubURI(searchBaseURL, endpoint), "documents", logger);
    ...
    public void process(Exchange exchange) throws Exception {
        ...
        if (serviceViolationsElement == null || serviceViolationsElement.getAsJsonArray().size() == 0) {
            ...
            logger.debug("Service integrity validation event processing for event with ID " + id
                    + " as a Pass due to no violations.");
        } else {
            serviceValidation.addProperty("result", "Fail");
            logger.debug("Service integrity validation event processing for event with ID " + id
                    + " as a Fail due to one or more violations.");
        }
    ...
    private JsonObject buildViolation(JsonObject validation, JsonObject violation) {
        ...
        formattedViolation.addProperty("modelVersionId", validation.get("entity").getAsJsonObject().get("poa-event")
                .getAsJsonObject().get("modelVersionId").getAsString());
        formattedViolation.addProperty("modelInvariantId", validation.get("entity").getAsJsonObject().get("poa-event")
                .getAsJsonObject().get("modelInvariantId").getAsString());
        formattedViolation.addProperty("serviceInstanceId", validation.get("entity").getAsJsonObject().get("poa-event")
                .getAsJsonObject().get("serviceInstanceId").getAsString());
    ...
    private JsonObject buildValidation(JsonObject validation) {
        ...
        formattedValidation.addProperty("modelVersionId", validation.get("entity").getAsJsonObject().get("poa-event")
                .getAsJsonObject().get("modelVersionId").getAsString());
        formattedValidation.addProperty("modelInvariantId", validation.get("entity").getAsJsonObject().get("poa-event")
                .getAsJsonObject().get("modelInvariantId").getAsString());
        formattedValidation.addProperty("serviceInstanceId", validation.get("entity").getAsJsonObject().get("poa-event")
                .getAsJsonObject().get("serviceInstanceId").getAsString());

        try {
            formattedValidation.addProperty("modelName",
                    validation.get("entity").getAsJsonObject().get("context-list").getAsJsonObject().get("sdc")
                            .getAsJsonObject().get("service").getAsJsonObject().get("name").getAsString());
            logger.debug(
                    "model name: " + validation.get("entity").getAsJsonObject().get("context-list").getAsJsonObject()
                            .get("sdc").getAsJsonObject().get("service").getAsJsonObject().get("name").getAsString());
